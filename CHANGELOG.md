...
- **Canonical Zero-Tolerance Blocking Architecture:** Enforced zero-tolerance pre-commit blocking in `cmd/crenox/commands/run.go`. Any finding, regardless of severity (`CRITICAL`, `HIGH`, `MEDIUM`, or `LOW`), halts execution and returns exit code 1. Bypass is permitted exclusively via explicit `crenox:ignore` inline comments or `.crenox.yaml` whitelist.
- **Fail-Closed Scan Integrity & Unified Incomplete Reporting:** Eliminated silent bypasses for unreadable files, permission errors, and broken Git pipes in `cmd/crenox/commands/run.go` and `scan.go`. Implemented unified `PrintIncomplete` across JSON, Plain, Pretty, SARIF, and GitLab SAST reporters (`internal/reporter/reporter.go`). In JSON, outputs a single valid object containing both `failed_files` and `findings` with `status: "scan_error"` (preventing dual JSON collisions). In SARIF, emits valid schema with `executionSuccessful: false` and error notifications. Exits strictly with code 1 under fail-closed security.
- **Strict OTA Self-Updater SHA-256 Verification:** Added strict 64-character hexadecimal SHA-256 checksum verification (`^[a-fA-F0-9]{64}$`) in `cmd/crenox/commands/update.go`. Updates abort immediately and remove temporary files if checksums are missing or mismatched, with a `--skip-verify` bypass flag for air-gapped scenarios.
- **Strict CLI Positional Argument Validation:** Hardened `run`, `install`, and `update` commands with `cobra.NoArgs`, preventing unintended command invocation or ignored argument errors.
