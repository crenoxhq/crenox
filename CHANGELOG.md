...
- **Fail-Closed Scan Integrity:** Eliminated silent bypasses for unreadable files, permission errors, and failed Git operations in `cmd/crenox/commands/run.go` and `scan.go`. Implemented `FailedFile` tracking and `PrintIncomplete` across JSON, Plain, and Pretty reporters (`internal/reporter/reporter.go`), exiting with code 1 (`scan_error`). A clean status (`exit 0`) is emitted only when 100% of eligible files have been scanned with zero findings.
